...
const AutomataCanvasInner: React.FC<AutomataCanvasProps> = ({
  ...
  const onConnect = useCallback(
    (params: Connection) => {
      ...
      const updatedAlphabet = Array.from(
        new Set([...automaton.alphabet, sym === 'ε' ? '0' : sym])
      );
  ...
  const handleAddTransitionSubmit = (e: React.FormEvent) => {
    ...
    const updatedTransitions = [...automaton.transitions, newTrans];
    const updatedAlphabet = Array.from(
      new Set([...automaton.alphabet, transSymbol === 'ε' ? '0' : transSymbol])
    );
  ...
  return (
    <div className="relative w-full h-[450px] min-h-[450px] rounded-2xl glass-panel border border-cyan-950/60 overflow-hidden shadow-2xl flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-slate-800 bg-slate-950/90 backdrop-blur z-10">
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-cyan-400">
            {title || `${automaton.type} Visual Canvas`}
          </span>
        ...
        {!readOnly && (
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-300 border border-cyan-500/40 rounded-lg text-xs font-mono transition-all"
            >
            ...
            <button
              onClick={() => {
                setTransFrom(automaton.states[0] || '');
                setTransTo(automaton.states[0] || '');
                setShowAddTrans(true);
              }}
              disabled={automaton.states.length === 0}
              className="flex items-center gap-1 px-2.5 py-1 bg-indigo-500/20 hover:bg-indigo-500/30 text-indigo-300 border border-indigo-500/40 rounded-lg text-xs font-mono transition-all disabled:opacity-40"
            >
              <Plus className="w-3.5 h-3.5" />
            ...
            {selectedNodeId && (
              <>
                <button
                  ...
                  className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-lg text-xs font-mono"
                  title="Make Start State"
                >
                ...
                <button
                  ...
                  className="px-2 py-1 bg-rose-950/60 hover:bg-rose-900/80 text-rose-300 border border-rose-800/60 rounded-lg text-xs font-mono"
                  title="Delete State"
                >
      ...
      {/* Main Flow Canvas Container with explicit height */}
      ...
      {showAddTrans && (
        <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm z-30 flex items-center justify-center p-4">
          <form
            ...
            className="w-full max-w-xs glass-panel p-5 rounded-2xl border border-cyan-500/40 space-y-4 shadow-2xl"
          >
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                type="submit"
                className="px-4 py-1.5 bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-mono font-bold text-xs rounded-lg shadow"
              >
                Add Transition
              </button>
            </div>
